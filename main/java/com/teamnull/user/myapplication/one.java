...
public class one extends AppCompatActivity {
    ...
    private void Start_Setting(){
        ...
        bitmap = BitmapFactory.decodeByteArray(db_data[i].Get_Bitmap2(), 0, db_data[i].Get_Bitmap2().length);
        display_right.setImageBitmap(bitmap);
        ...
        tts = new TextToSpeech(this, new TextToSpeech.OnInitListener() {
            ...
            public void onInit(int status) {
                ...
                if(status != ERROR){
                    tts.setSpeechRate(1.0f);                //언어 속도
                    tts.setLanguage(Locale.UK);            //언어 선택(영국)
                }
                TTS();
            }
        });
        ...
        next.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) { //홈키 버튼이벤트: 메인UI로 이동
                ...
                if(i<25) {
                    ...
                    display.setImageBitmap(bitmap);
                    bitmap = Byte_To_Bitmap(db_data[i].Get_Bitmap2());
                    display_right.setImageBitmap(bitmap);

                }if(i==25) {
                    Toast.makeText(getApplicationContext(), "마지막 알파벳입니다.\n홈버튼을 눌러주세요.", Toast.LENGTH_SHORT).show();
    ...
    private void TTS(){
        ...
        //ei
        if(i == 0)  tts.speak("ei",TextToSpeech.QUEUE_FLUSH,null);  //읽기
        else        tts.speak(str[i].split("/")[0],TextToSpeech.QUEUE_FLUSH,null);
